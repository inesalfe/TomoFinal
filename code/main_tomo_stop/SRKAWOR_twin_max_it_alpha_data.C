...
int main (int argc, char *argv[]) {
	...
	if(argc != 9 && argc != 10) {
		...
		cout << "'./bin/SRKAWOR_twin_max_it_alpha_data.exe <data_set> <n_runs> <M> <N> <it_per_thread> <alpha> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/tomo_stop/" + matrix_type + "/SRKAWOR_twin_alpha_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + str_alpha + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/tomo_stop/" + matrix_type + "/SRKAWOR_twin_alpha_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + str_alpha + "_" + to_string(max_it_stop);
