...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/SRKWOR_box_proj_max_it_data.exe <data_set> <n_runs> <M> <N> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/tomo_stop/" + matrix_type + "/SRKWOR_twin_box_proj_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/tomo_stop/" + matrix_type + "/SRKWOR_twin_box_proj_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
