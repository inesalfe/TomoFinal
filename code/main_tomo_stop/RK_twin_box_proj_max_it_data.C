...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/RK_box_proj_max_it_data.exe <data_set> <n_runs> <M> <N> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		mt19937 generator_down(run+1);
		mt19937 generator_up(n_runs+run+1);
		it = 0;
		storage_counter = 0;
		start = omp_get_wtime();
		while(it < max_it_stop) {
			it++;
			line = dist(generator_down);
	...
	string filename_error = "errors/tomo_stop/" + matrix_type + "/RK_twin_box_proj_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/tomo_stop/" + matrix_type + "/RK_twin_box_proj_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
