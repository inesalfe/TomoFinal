...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/CK_max_it_data.exe <data_set> <n_runs> <M> <N> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/tomo_stop/" + matrix_type + "/CK_twin_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/tomo_stop/" + matrix_type + "/CK_twin_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
