...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/RKA_pos_proj_csr_parallel_max_it_dist_data.exe <data_set> <n_runs> <M> <N> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id) firstprivate(it)
		{
			...
			discrete_distribution<> dist(sqrNorm_line.begin()+BLOCK_LOW(t_id, num_threads, M), sqrNorm_line.begin()+BLOCK_LOW(t_id, num_threads, M)+BLOCK_SIZE(t_id, num_threads, M));
			while(it < max_it_stop) {
				...
				#pragma omp for
					for (int i = 0; i < N; i++)
						x_prev[i] = x_k[i];
				line = BLOCK_LOW(t_id, num_threads, M)+dist(generator);
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/RKA_pos_proj_dist_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(max_it_stop);
	string filename_res = "errors/omp_sparse/" + matrix_type + "/RKA_pos_proj_dist_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(max_it_stop);
