...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/SRKAWOR_pos_proj_csr_max_it_dist_data.exe <data_set> <n_runs> <M> <N> <it_per_thread> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(it < max_it_stop) {
			...
			for (int i = 0; i < it_per_thread; i++) {
				line = samp_line[BLOCK_LOW(i, it_per_thread, M) + it%BLOCK_SIZE(i, it_per_thread, M)];
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/SRKAWOR_pos_proj_dist_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(max_it_stop);
	string filename_res = "errors/omp_sparse/" + matrix_type + "/SRKAWOR_pos_proj_dist_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(max_it_stop);
