...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/SRKAWOR_box_proj_csr_parallel_max_it_atomic_no_barrier.exe <data_set> <n_runs> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/omp_sparse/" + matrix_type + "/SRKAWOR_box_proj_atomic_no_barrier_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(max_it_stop);
