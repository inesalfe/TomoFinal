...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/SRKAWOR_csr_parallel_max_it_just_atomic.exe <data_set> <n_runs> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/omp_sparse/" + matrix_type + "/SRKAWOR_just_atomic_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(max_it_stop);
