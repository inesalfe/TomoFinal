...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/RKA_csr_parallel_max_it_dist_atomic_no_barrier.exe <data_set> <n_runs> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		it = 0;
		start = omp_get_wtime();
		#pragma omp parallel private(line, scale, t_id, index, col) firstprivate(it)
		{
	...
	string filename_sol = "outputs/omp_sparse/" + matrix_type + "/RKA_dist_atomic_no_barrier_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(max_it_stop);
