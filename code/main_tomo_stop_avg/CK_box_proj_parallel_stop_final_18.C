...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/CK_box_proj_parallel_stop_final_18.exe <data_set> <n_runs> <M> <N> <min_it> <bucket_size>'" << endl;
		exit(1);
	}
				...
				while(!solution_found) {
					...
					while (line_down < M) {
						...
						while (index < row_idx[line_down+1]) {
							...
							x_down[col] = x_down[col] * (x_down[col] > 0) * (x_down[col] <= 1) + (x_down[col] > 1);
							index++;
						}
						...
						while (index < row_idx[line_up+1]) {
							...
							x_up[col] = x_up[col] * (x_up[col] > 0) * (x_up[col] <= 1) + (x_up[col] > 1);
							index++;
						}
						line_up -= num_threads;
						iner_it += num_threads;
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/CK_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads);
