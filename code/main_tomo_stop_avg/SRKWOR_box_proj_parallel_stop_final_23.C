...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/SRKWOR_box_proj_parallel_stop_final_23.exe <data_set> <n_runs> <M> <N> <min_it> <bucket_size>'" << endl;
		exit(1);
	}
	...
	int sol_block_for = ceil(N/num_threads);
	...
	for(int run = 0; run < n_runs; run++) {
		...
		it = 0;
		start = omp_get_wtime();
		#pragma omp parallel private(line_down_idx, line_up_idx, line_down, line_up, scale, t_id, iner_it, col, index) firstprivate(it)
		{
			t_id = omp_get_thread_num();
			iner_it = t_id;
			if (t_id == num_threads_total-1) {
				...
				while(!solution_found) {
					...
					#pragma omp for schedule(static, sol_block_for) nowait
						for (int i = 0; i < N; i++) {
							if (x_down[i] < 0)
								x_down[i] = 0;
							else if (x_down[i] > 1)
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/SRKWOR_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads_total);
