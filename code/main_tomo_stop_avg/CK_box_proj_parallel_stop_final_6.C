...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/CK_box_proj_parallel_stop_final_6.exe <data_set> <n_runs> <M> <N> <min_it> <bucket_size>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line_down, line_up, scale, t_id, iner_it, col, index) firstprivate(it)
		{
			...
			while(!solution_found) {
				...
				#pragma omp for
					for (int i = 0; i < N; i++) {
						x_down[i] = x_down[i] * (x_down[i] > 0) * (x_down[i] <= 1) + (x_down[i] > 1);
						x_up[i] = x_up[i] * (x_up[i] > 0) * (x_up[i] <= 1) + (x_up[i] > 1);
					}
				#pragma omp single
				{
