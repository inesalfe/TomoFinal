...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/CK_box_proj_csr_max_it_alpha_data.exe <data_set> <n_runs> <M> <N> <alpha> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/seq_sparse/" + matrix_type + "/CK_alpha_box_proj_error_" + to_string(M) + "_" + to_string(N) + "_" + str_alpha + "_" + to_string(max_it_stop);
	string filename_res = "errors/seq_sparse/" + matrix_type + "/CK_alpha_box_proj_res_" + to_string(M) + "_" + to_string(N) + "_" + str_alpha + "_" + to_string(max_it_stop);
