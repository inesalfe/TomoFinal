...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/RK_box_proj_csr_max_it_sol.exe <data_set> <n_runs> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/seq_sparse/" + matrix_type + "/RK_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(max_it_stop);
